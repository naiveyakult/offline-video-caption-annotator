...
describe("macOS v0.4.1 release contract", () => {
  it("builds the tagged source on an Apple Silicon runner", () => {
    ...
    expect(workflow).toContain('tags: ["v0.4.1"]');
    expect(workflow).toContain("ref: v0.4.1");
  ...
  it("verifies and uploads only the macOS v0.4.1 assets", () => {
    ...
    expect(workflow).toContain("RELEASE_TAG: v0.4.1");
