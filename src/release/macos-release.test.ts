...
describe("macOS v0.6.0 libmpv release contract", () => {
  it("builds the release source and its pinned patched LGPL libmpv runtime", () => {
    ...
    expect(workflow).toContain("ref: v0.6.0");
  ...
  it("verifies bundled dylibs and uploads a public macOS release", () => {
    ...
    expect(workflow).toContain("offline-video-caption-annotator_0.6.0_macos_aarch64.dmg");
    expect(workflow).toContain("offline-video-caption-annotator_0.6.0_macos_aarch64.dmg.sha256");
    ...
    expect(readme).toContain("offline-video-caption-annotator_0.6.0_macos_aarch64.dmg");
  ...
  it("runs for the formal tag and remains manually rerunnable", () => {
    ...
    expect(workflow).toContain("MACOS_BUILD_SHA: ${{ needs.build-macos.outputs.source_sha }}");
    expect(workflow).toContain("构建提交 ${MACOS_BUILD_SHA}");
    expect(workflow).not.toContain("构建提交 ${GITHUB_SHA}");
  });
