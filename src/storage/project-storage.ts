...
function recordsStatus(task: Pick<ProjectTask, "document" | "records" | "drafts" | "error" | "mediaAnomaly" | "videoDecision">): ProjectTask["status"] {
...
function mergeSavedState(tasks: ProjectTask[], saved?: ProjectSnapshot): ProjectTask[] {
  return tasks.map((task) => {
    ...
    const allTrue = records && Object.keys(records).length === validUnitIds.size &&
