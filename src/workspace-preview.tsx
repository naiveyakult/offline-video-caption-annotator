...
export function Preview() {
  ...
  const task: ProjectTask = {
    ...
    document: source,
    status: Object.keys(records).length === 9 ? "complete" : Object.keys(records).length ? "in_progress" : "not_started",
    records,
    drafts: {},
    videoPosition: 0,
  };
  return (
    <AnnotationWorkspace
      ...
      onCommit={(unitId, decision) => {
        setRecords((current) => ({
          ...current,
          [unitId]: { unitId, decision, correctedFields: {}, updatedAt: new Date().toISOString() },
        }));
      }}
    />
  );
