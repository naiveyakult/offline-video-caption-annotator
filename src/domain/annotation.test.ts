...
describe("bilingual caption annotation domain", () => {
  it("exports a binary early-stop audit without changing the source Caption", () => {
    ...
    const records: Record<string, AnnotationRecord> = {
      ...
      "overview.overall_audio_style": {
        ...
        decision: "false",
        correctedFields: { overall_audio_style: "The source value." },
    ...
    expect(meta).toMatchObject({
      ...
      stopped_at_unit_id: "overview.overall_audio_style",
      counts: { source_total: 9, annotated: 2, true: 1, false: 1, unreviewed: 7 },
    });
    expect(meta.units).toHaveLength(2);
    expect(meta.units.map((unit) => unit.unit_id)).toEqual([
      "overview.overall_visual_style",
      "overview.overall_audio_style",
    ]);
  ...
  it("exports an all-True audit after every unit is labeled", () => {
    ...
    const records = Object.fromEntries(buildAnnotationUnits(document).map((unit, index) => [
      unit.id,
      {
        ...
        correctedFields: {},
        updatedAt: `2026-07-27T00:00:${String(index).padStart(2, "0")}.000Z`,
      },
    ]));
    ...
    expect(meta.stopped_at_unit_id).toBeNull();
